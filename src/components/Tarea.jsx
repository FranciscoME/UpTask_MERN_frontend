  ...
  return (
    <div className='border-b p-5 flex justify-between items-center'>
      <div className='flex flex-col items-start'>
        ...
        <p className='text-sm mb-2 text-gray-700 uppercase'>{descripcion}</p>
        <p className='text-xl mb-2'>{formatearFecha(fechaEntrega)}</p>
        <p className='text-gray-600 mb-2'>Prioridad: {prioridad}</p>
        {estado && <p className='text-xs bg-green-400 uppercase p-2 rounded-lg text-white text-center font-bold'>Completada por: {tarea?.completado.nombre}</p>}
      </div>
